<#
    This script runs after Windows starts up.
...
$requireCreateLog = $false
$createLogSucceed = $false
...
try
{
    ...
    if (-not $log)
    {
        WriteToWindowsLog CreatingHostAccessLog
        ...
        {
            WriteToWindowsLog FailedToCreateHostAccessLog -Data ($_ | Out-String)
            $hasError = $true
        }
        finally
        {
            if ((-not $hasError) -and (-not $log))
            {
    ...
    WriteToWindowsLog WindowsStartupScriptBegin
...
{
    WriteToWindowsLog WindowsStartupScriptError -Data ($_ | Out-String)
    $hasError = $true
}
finally
{
    if ($hasError)
    {
        WriteToWindowsLog WindowsStartupScriptEndWithError
    }
    else
    {
        WriteToWindowsLog WindowsStartupScriptEnd
    }
}
